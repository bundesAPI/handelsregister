...
class DetailsParser:
    ...
    CAPITAL_PATTERN = re.compile(
        r"(?:Stamm|Grund)kapital[:\s]*([0-9.,]+)\s*(EUR|€|DM)?", re.IGNORECASE
    )
    ...
    def _extract_representatives_from_text(cls, text: str) -> list[Representative]:
        ...
        patterns = [
            ...
            (r"Vorstand[:\s]*([A-ZÄÖÜ][a-zäöüß]+(?:\s+[A-ZÄÖÜ][a-zäöüß]+)+)", "Vorstand"),
            (r"Prokurist(?:in)?[:\s]*([A-ZÄÖÜ][a-zäöüß]+(?:\s+[A-ZÄÖÜ][a-zäöüß]+)+)", "Prokurist"),
            (
